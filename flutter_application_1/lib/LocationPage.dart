...
class _MyHomePageState extends State<LocationPage> {
  ...
  Future<void> _addAllMarkers() async {
    await _addTrashMarkers(
        '1'); // tilføjer alle røde genbrugspladsers markør fra fil 1
  ...
  void _onMarkerTapped(LatLng position, String name, String openHours) {
    showDialog(
      // åbner et lille vindue foran kortet
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          actions: [
            TextButton(
              // laver en knap til at forlade det lille vindue.
              child: const Text('OK'),
              onPressed: () {
  ...
  Future<void> _addTrashMarkers(String choosen) async {
    String export =
        'assets/export-$choosen.geojson'; // definere en filsti til den valgte fil med tallet choosen
    try {
      ...
      setState(() {
        for (var feature in features) {
          // går alle featuresne igennem en ad gangen.
          final geometry = feature['geometry']; // finder geometry i featuren
          final coordinates =
              geometry['coordinates']; //finder koordinaterne i geometry
          final properties =
              feature['properties']; // finder properties i featuren
          LatLng? firstCoordinate;
          String? siteName;
          String? openHours;
          siteName = properties['name'] ??
              'Genbrugsplads'; //hvis der findes et navn så gem navnet, ellers 'genbrugsplads'
          openHours = properties['opening_hours'] ??
              'Ukendt'; //hvis der findes åbningstider så gem dem, ellers 'ukendt'

          if (geometry['type'] == 'Point') {
            // hvis der kun er et koordinat så laver vi en markør i dette punkt
            firstCoordinate = LatLng(coordinates[1], coordinates[0]);
          } else if (geometry['type'] == 'LineString' ||
              geometry['type'] == 'Polygon') {
            // hvis der er flere koordinater
            ...
            if (choosen == '1') {
              _markers1.add(Marker(
                ...
                child: GestureDetector(
                  ...
                  child: const Icon(
                    Icons
                        .recycling, // de bliver vist som røde genbrugs symboler
                    color: Colors.red,
                    size: 35.0,
                  ),
                ),
              ...
              _markers2.add(
                Marker(
                  // andet dokument bliver vist som små grønne skraldespande
                  width: 80.0,
                  height: 80.0,
                  point: firstCoordinate,
                  child: const Icon(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        // standart titel og baggrundsfarve
        title: const Text(
            ...
            Expanded(
              child: FlutterMap(
                ...
                children: [
                  ...
                  MarkerLayer(
                    // tilføjer de to forskellige grupper af markør
                    markers: _markers1,
                  ),
                  MarkerLayer(
                    markers: _markers2,
                  ...
                  Positioned(
                    // tilføjer en info box der forklare hvad der er hvad
                    left: 6,
                    top: 6,
                    child: Container(
                      ...
                      child: const Row(
                        // selve informationen insættes som standart
                        children: [
                          Row(
                            children: [
                              Icon(
